...
int manual_relay_control = -1; // -1: auto, 0: off, 1: on
...
void control_water_level(uint16_t* water_level) {
    *water_level = mraa_aio_read(adc_aio);
    ...
    if (manual_relay_control == -1) {
        mraa_gpio_write(relay_gpio, *water_level > 700 ? 0 : 1);
    } else {
        mraa_gpio_write(relay_gpio, manual_relay_control == 1 ? 0 : 1);
    }
}
...
void MQTT_Init() {
    ...
    cmd = "CMD+MQTTCONCFG=3,mqtt-aayushisamantsinghar21-sbfpla,,,,,,,,,\r\n";
    mraa_uart_write(uart, cmd, strlen(cmd));
    sleep(1);
...
void process_rightec_commands() {
    ...
    if (len > 0) {
        if (strstr(buffer, "relay=1")) {
            manual_relay_control = 1;
        } else if (strstr(buffer, "relay=0")) {
            manual_relay_control = 0;
        } else if (strstr(buffer, "relay=auto")) {
            manual_relay_control = -1;
        }
