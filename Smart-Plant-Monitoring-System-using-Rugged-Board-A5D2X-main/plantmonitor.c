...
void move_servo_soft_pwm(float angle) {
    ...
    float pulse_ms = 1.0 + (angle / 270.0); // Map to 1–2ms
...
void MQTT_Init() {
    ...
    cmd = "CMD+MQTTSUB=base/relay/control\r\n";
    mraa_uart_write(uart, cmd, strlen(cmd));
    sleep(1);

    cmd = "CMD+MQTTSUB=base/servo/control\r\n";
    mraa_uart_write(uart, cmd, strlen(cmd));
    sleep(1);
...
void check_uart_commands() {
    ...
    if (strstr(buffer, "base/relay/control") && strstr(buffer, "ON")) {
        mraa_gpio_write(relay_gpio, 0);
        printf("Relay turned ON via MQTT\n");
    } else if (strstr(buffer, "base/relay/control") && strstr(buffer, "OFF")) {
        mraa_gpio_write(relay_gpio, 1);
        printf("Relay turned OFF via MQTT\n");
    } else if (strstr(buffer, "base/servo/control") && strstr(buffer, "ON")) {
        move_servo_soft_pwm(90);
        printf("Servo moved to 90° via MQTT\n");
    } else if (strstr(buffer, "base/servo/control") && strstr(buffer, "OFF")) {
        move_servo_soft_pwm(0);
        printf("Servo moved to 0° via MQTT\n");
    }
}
...
int main() {
    ...
    while (1) {
        ...
        check_uart_commands(); // Check MQTT commands from WE10
