...
// === Pin Definitions ===
#define ADC_SOIL_PIN        6    // Single ADC pin used for soil moisture
#define RELAY_PIN           14
...
void read_soil_moisture(uint16_t* value);
void control_pump_by_soil(uint16_t soil_val);
void WE10_Init();
void MQTT_Init();
...
void init_peripherals() {
    ...
    adc_soil = mraa_aio_init(ADC_SOIL_PIN);
    ...
    if (!relay_gpio || !servo_gpio || !adc_soil || !i2c || !uart) {
...
void control_pump_by_soil(uint16_t soil_val) {
    if (soil_val > 700) {
        ...
        printf("Pump ON (Dry Soil)\n");
    } else {
        mraa_gpio_write(relay_gpio, 0);
        printf("Pump OFF (Wet Soil)\n");
    }
}
...
int main() {
    ...
    while (1) {
        ...
        read_soil_moisture(&soil_value);
        control_pump_by_soil(soil_value);

        char soil_str[16];
        snprintf(soil_str, sizeof(soil_str), "%u", soil_value);
        MQTT_Publish("base/sensor/soil", soil_str);
